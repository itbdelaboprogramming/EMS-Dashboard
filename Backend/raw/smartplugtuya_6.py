...
Device_Id = 'eb1de2818463ccdfb61qui' # Device Id from Tuya device sensor
Address_Id = '192.168.18.14' # IP Address connected to Tuya device sensor
Local_Key = '483fe6fb7fa49e21' # Local Key generated from  python -m tinytuya wizard
...
def get_controller_status():
    try:
        ...
        query = "SELECT status FROM controller WHERE id = 6"
...
def get_sum_electricity():
    try:
        connection = mysql.connector.connect(**db_config)
        cursor = connection.cursor(dictionary=True)
        query = "SELECT SUM(electricity) as electricity FROM `tuya_smart_plug_6`"
        cursor.execute(query)
        result = cursor.fetchone()
        if result:
...
while True:
    ...
    if status == "online":
        ...
        #     # query = "SELECT status FROM controller WHERE id = 8"
